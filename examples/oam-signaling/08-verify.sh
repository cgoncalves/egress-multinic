...
if oc get pod -n demo-egressip gw-egressip-pod &>/dev/null; then
    ...
    echo "Expected source: 192.168.150.200 (SNAT /32 -- all ovn-k8s-mp0 traffic on gateway)"
    ...
    echo "Expected source: 192.168.150.10 (masquerade -- not in egressip-pods set)"
